...
// ============ IMPORTER VOTRE EXPORTPANEL ============
const ExportPanel = lazy(() => import('./components/export/ExportPanel.jsx'));
  ...
  if (checking) {
    return (
      <div className="fixed top-4 right-4 z-50 flex items-center gap-2 px-3 py-1.5 bg-yellow-500/20 text-yellow-300 rounded-full border border-yellow-500/30 backdrop-blur-sm">
        ...
        <span className="text-xs font-medium">Vérification connexion...</span>
      </div>
    );
  }
  ...
  if (!connected) {
    return (
      <div className="fixed top-4 right-4 z-50 flex items-center gap-2 px-3 py-1.5 bg-red-500/20 text-red-300 rounded-full border border-red-500/30 backdrop-blur-sm animate-pulse">
        ...
        <span className="text-xs font-medium">Déconnecté du serveur</span>
      </div>
    );
  }
...
// Layout sans navbar pour plein écran
...
// Composant de superposition pour les déconnexions
        ...
        <div className="space-y-4">
          ...
          {showDetails && (
            <div className="mt-4 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
              <h4 className="text-sm font-medium text-gray-300 mb-2">Dépannage :</h4>
              <ul className="text-sm text-gray-400 space-y-1">
                <li>• Vérifiez que le serveur backend est démarré</li>
                <li>• Vérifiez votre connexion internet</li>
                <li>• Vérifiez l'URL de l'API dans .env</li>
                <li>• Les ports 5000 (backend) et 5173 (frontend) doivent être disponibles</li>
              </ul>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">Cette opération peut prendre quelques instants</p>
        </div>
      </div>
    );
  }
...
const RoleRoute = ({ children, roles = [] }) => {
  ...
  if (!user || !roles.includes(user.role)) {
    console.log(`🚫 Accès non autorisé. Rôle ${user?.role} requis: ${roles.join(', ')}`);
...
const AppRoutes = () => {
  ...
  return (
    <Routes>
      ...
      {/* ========== ROUTES RÉINITIALISATION - ORDRE CORRECT ========== */}
      {/* 1. AVEC token (DOIT ÊTRE EN PREMIER) */}
      ...
      {/* ========== NOUVELLE ROUTE : EXPORTATION ========== */}
      <Route path="/exports" element={
        <PrivateRoute>
          <RoleRoute roles={['admin', 'manager', 'accounting']}>
            <MainLayout>
              <div className="max-w-6xl mx-auto">
                {/* Header simple */}
                <div className="mb-8">
                  <h1 className="text-3xl font-bold text-gray-800">Centre d'Exportation</h1>
                  <p className="text-gray-600 mt-2">
                    Exportez vos données au format Excel, PDF ou ZIP
                  </p>
                </div>
                
                {/* Votre ExportPanel existant */}
                <ExportPanel />
                
                {/* Section d'information supplémentaire */}
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white p-6 rounded-xl border border-gray-200">
                    <h3 className="font-bold text-gray-800 mb-3">Formats disponibles</h3>
                  ...
                  <div className="bg-white p-6 rounded-xl border border-gray-200">
                    <h3 className="font-bold text-gray-800 mb-3">Types de données</h3>
                    <ul className="space-y-2">
                      <li className="flex items-center text-gray-600">
                  ...
                  <div className="bg-white p-6 rounded-xl border border-gray-200">
                    ...
                    <ul className="space-y-2 text-sm text-gray-600">
                      <li>• Sélectionnez des périodes réalistes</li>
                      <li>• Utilisez les filtres pour réduire le volume</li>
                      <li>• Exportez régulièrement pour sauvegarde</li>
                      <li>• Vérifiez votre connexion internet</li>
                    </ul>
                  </div>
                </div>
      ...
      {/* NOUVELLE ROUTE : Mes fiches de paie (pour employés seulement) */}
      <Route path="/my-payslips" element={
        <PrivateRoute>
          {/* Cette route est réservée aux employés */}
          {user?.role === 'employee' ? (
            <MainLayout>
              <EmployeePayroll />
            </MainLayout>
          ) : (
            // Si un admin ou manager essaie d'y accéder, rediriger vers la page admin
            <Navigate to="/payroll/payslips" replace />
          )}
        </PrivateRoute>
      } />
      
      {/* Route pour visualiser une fiche de paie spécifique */}
      <Route path="/payroll/payslip/:employeeId/:monthYear" element={
        <PrivateRoute>
          {/* Les employés peuvent voir leur propre fiche, les admins peuvent voir toutes */}
          {(user?.role === 'employee') ? (
            <MainLayout>
              <PayslipPage />
            </MainLayout>
      ...
      {/* Routes faciales supplémentaires */}
          ...
          <div className="text-center max-w-md">
            ...
            <div className="mt-8 pt-6 border-t border-gray-700">
              <p className="text-gray-500 text-sm">
                Routes disponibles: /login, /dashboard, /employees, /attendance, /attendance/quick-check, /attendance/correction/:employeeId, /facial-attendance, /payroll, /payroll/payslips, /my-payslips, /reports, /settings, /profile, /notifications, /exports
              </p>
            </div>
          </div>
        </div>
...
const App = () => {
  // État global pour suivre la connexion backend
      ...
      try {
        ...
        if (mounted) {
          ...
          if (isConnected && !backendConnected) {
            console.log('✅ Connexion backend rétablie');
          }
        }
      } catch (error) {
        if (mounted) {
