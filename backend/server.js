...
// Utilisation du parser natif d'Express
app.use(express.json({ limit: '10mb' }));
app.use(express.urlencoded({ extended: true, limit: '10mb' }));

// ==================== ROUTE DE DIAGNOSTIC ULTIME ====================
// Placez cette route ICI, avant tous les autres middlewares de debug
...
app.use((req, res, next) => {
  ...
  // Ajouter un timestamp à la requête pour le tracking
...
// ==================== ROUTE DE DEBUG POUR LISTER LES ROUTES ====================
app.get('/api/debug/routes', (req, res) => {
  ...
  app._router.stack.forEach(middleware => {
    if (middleware.route) {
      ...
      // Routes montées avec router.use
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log('   GET  /api/debug/routes    → Lister toutes les routes');
      ...
      console.log('   POST /api/auth/login-debug → Route de debug');
      ...
      console.log('\n🔧 ROUTE DE DEBUG AJOUTÉE: /api/auth/login-debug');
    });
